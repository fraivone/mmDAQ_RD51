...
CGuiTabChamberEvent1D::CGuiTabChamberEvent1D(CGuiBase* gui_parent,
...
CGuiTabChamberEvent1D::~CGuiTabChamberEvent1D()
{
...
void CGuiTabChamberEvent1D::do_construct_tab()
{
...
void CGuiTabChamberEvent1D::do_update(const CDetBase* detbase)
{
...
//   detbase->extract_readouts(all_readouts);
   ...
   // std::cout<<"all readouts size "<<all_readouts.size()<<std::endl;
   ...
   BOOST_FOREACH(const CDetReadoutBase* rd, all_readouts) {
      ...
      //std::pair<double,double> tr = rd->get_full_time_range(); //wrong time range
     
      std::pair<double,double> tr = rd->get_time_range();
      int tbins =  (tr.second-tr.first) /25;
      
      //std::cout << "CGuiTabChamberEvent1D::get_time trange=" << tr.first << " " << tr.second << std::endl;
      //std::cout << "CGuiTabChamberEvent1D:: time bin =" << tbins << std::endl;
      ...
      //                          tbins, tr.first, tr.second);

      if(m_tdisp.back()->GetEntries() !=0) m_tdisp.back()->SetBins((int)sr.second - (int)sr.first - 1, sr.first, sr.second,
			      tbins, tr.first, tr.second);

      else  m_tdisp.back()->SetBins((int)sr.second - (int)sr.first - 1, sr.first, sr.second,
			      tbins+1, tr.first, tr.second);
...
void CGuiTabChamberEvent1D::do_clear_tab()
{
   m_qdisp.clear();
   m_tdisp.clear();
}
